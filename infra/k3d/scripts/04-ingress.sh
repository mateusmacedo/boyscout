...
revert_centralized_ingress() {
    ...
    echo -e "  - ${GREEN}Keycloak:${NC} https://auth.boyscout.local"
    echo -e "  - ${GREEN}LocalStack:${NC} https://aws.boyscout.local"
    echo -e "  - ${GREEN}RabbitMQ:${NC} https://rabbitmq.boyscout.local"
    echo ""
    echo -e "${BLUE}[info] Lembre-se de atualizar o /etc/hosts com os domínios individuais:${NC}"
    echo -e "  ${YELLOW}127.0.0.1 auth.boyscout.local${NC}"
    echo -e "  ${YELLOW}127.0.0.1 aws.boyscout.local${NC}"
    echo -e "  ${YELLOW}127.0.0.1 rabbitmq.boyscout.local${NC}"
}
