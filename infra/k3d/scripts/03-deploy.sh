...
deploy_infrastructure() {
    echo "[deploy] Deploying Liderança Sites infrastructure..."
    ...
    echo "[ok] Liderança Sites infrastructure deployed!"
    echo "[info] Access URLs:"
    echo "  - Keycloak: https://boyscout.local/auth/"
    echo "  - LocalStack: https://boyscout.local/aws/"
    echo "  - RabbitMQ: https://boyscout.local/rabbitmq/"
    echo "  - Health: https://boyscout.local/health"
    echo "  - Dashboard: https://boyscout.local/"
}
...
show_status() {
    ...
    # Check namespace
    echo "[namespace] Namespace boyscout:"
    kubectl get namespace boyscout 2>/dev/null || echo "  ✗ Namespace não encontrado"
    echo
    
    # Check pods
    echo "[pods] Pods no namespace boyscout:"
    kubectl get pods -n boyscout -o wide
    echo
    
    # Check services
    echo "[services] Services no namespace boyscout:"
    kubectl get services -n boyscout
    echo
    
    # Check ingress
    echo "[ingress] Ingress no namespace boyscout:"
    kubectl get ingress -n boyscout
    echo
    
    # Check PVCs
    echo "[storage] Persistent Volume Claims:"
    kubectl get pvc -n boyscout
    echo
    
    # Check deployments
    echo "[deployments] Deployments no namespace boyscout:"
    kubectl get deployments -n boyscout
    echo
    ...
    case "${service}" in
        "all")
            ...
            for deployment in postgresql redis keycloak rabbitmq localstack nginx; do
                if kubectl get deployment "${deployment}" -n boyscout >/dev/null 2>&1; then
                    echo "[logs] === ${deployment} ==="
                    kubectl logs -f deployment/"${deployment}" -n boyscout --tail=50
                    echo
                fi
            done
            ;;
        "postgresql"|"redis"|"keycloak"|"rabbitmq"|"localstack"|"nginx")
            if kubectl get deployment "${service}" -n boyscout >/dev/null 2>&1; then
                echo "[logs] Logs for ${service}:"
                kubectl logs -f deployment/"${service}" -n boyscout
            else
...
test_connectivity() {
    ...
    # Test namespace
    if ! kubectl get namespace boyscout >/dev/null 2>&1; then
        echo "[error] Namespace boyscout não encontrado"
        exit 1
    fi
    
    echo "[ok] Namespace boyscout encontrado"
    
    # Test pods readiness
    echo "[test] Checking pods readiness..."
    for deployment in postgresql redis keycloak rabbitmq localstack nginx; do
        if kubectl get deployment "${deployment}" -n boyscout >/dev/null 2>&1; then
            if kubectl get pods -n boyscout -l app.kubernetes.io/name="${deployment}" | grep -q "Running"; then
    ...
    if kubectl get ingress centralized-ingress -n boyscout >/dev/null 2>&1; then
