...
# Script para testar conectividade da infraestrutura Liderança Sites
...
echo "[test] Testando conectividade da infraestrutura Liderança Sites..."
...
test_postgresql() {
    ...
    if docker exec boyscout-postgresql pg_isready -U boyscout -d boyscout >/dev/null 2>&1; then
        ...
        if docker exec boyscout-postgresql psql -U boyscout -d boyscout -c "SELECT 1;" >/dev/null 2>&1; then
...
test_redis() {
    ...
    if docker exec boyscout-redis redis-cli ping >/dev/null 2>&1; then
        ...
        if docker exec boyscout-redis redis-cli set test_key "test_value" >/dev/null 2>&1; then
            if docker exec boyscout-redis redis-cli get test_key | grep -q "test_value"; then
                echo "✓ Redis: Operações OK"
                docker exec boyscout-redis redis-cli del test_key >/dev/null 2>&1
            else
...
test_rabbitmq() {
    ...
    if docker exec boyscout-rabbitmq rabbitmq-diagnostics ping >/dev/null 2>&1; then
        ...
        if curl -s -f http://localhost/rabbitmq/api/overview -u boyscout:boyscout123 >/dev/null 2>&1; then
